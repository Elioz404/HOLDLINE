...
async function main(): Promise<void> {
  ...
  try {
    ...
    // Run the batch first, so the verdicts — the thumbnail — can be shot first.
    ...
    await until(page, "the verdicts", () => document.querySelectorAll("#cards .card").length >= 3, 120_000);

    // 01 — the thumbnail, and the only image many readers will see. The
    // summary line reads in one glance and the first verified card sits
    // whole underneath it, so nothing important is cut off at either edge.
    await page.evaluate(() => document.getElementById("summary")?.scrollIntoView({ block: "start" }));
    await shoot(page, "answers-established-and-withheld",
      "Three places called. Five answers established. <i>One answer withheld</i>, with the reason attached.");

    // 02 — the withheld card, framed whole. Aligning its top to the frame and
    // then easing back keeps the header off the very edge; centring it cropped
    // the reason off the bottom, which is the half that matters.
    await page.evaluate(() => {
      document.querySelectorAll("#cards .card")[1]?.scrollIntoView({ block: "start" });
    ...
    // 05 — the wait, which is the part a person would otherwise be doing.
